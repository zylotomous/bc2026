...
REQUIRE_OPPOSITE_HEADS = False  # child_has_exit already verifies the child has a real place to
                                # go (checked against all 4 sides, not just one inherited
                                # direction), and ordinary collision-avoidance covers parent/child
                                # colliding later -- a direction label adds nothing on top of that
...
QUEEN_MIN_SPLIT_LENGTH = 0.12   # per-round growth rate feeding the length threshold below

# --- map-size-aware splitting tuning ---
SMALL_MAP_THRESHOLD = 25            # width and/or height at or below this counts as "small"
QUEEN_SMALL_MAP_LENGTH_CAP = 8      # small maps: queens never need to be longer than this to split
QUEEN_LARGE_MAP_LENGTH_CAP = 12     # large maps: same idea, just a bit more headroom
...
def queen_split_threshold(now):
    """Minimum length a queen needs before splitting for growth. Scales with the round number
    so early queens don't wait long, but is capped so queens settle around a target length
    instead of growing without bound -- the cap is lower on small maps, where we've rarely
    seen dragons get much past it anyway."""
    cap = QUEEN_SMALL_MAP_LENGTH_CAP if is_small_map else QUEEN_LARGE_MAP_LENGTH_CAP
    return min(QUEEN_MIN_SPLIT_LENGTH * now, cap)
...
def split_ready(body, heading):
    """Would a split of `body` leave the child facing a direction other than the parent's
    current heading? (Not necessarily opposite -- any non-parallel facing is enough, since the
    two dragons still end up moving apart rather than straight into each other.)"""
    if body is None or len(body) < 4:
        return False
    if not REQUIRE_OPPOSITE_HEADS:
        return True
    child_dir = dir_between(body[-2], body[-1])
    return child_dir is not None and child_dir != heading
...
def find_bigger_enemy_target(mp, my_length, my_team, enemies, enemy_lengths):
    """Among currently visible enemy heads whose estimated length is at least ours, pick the
    nearest one to hunt for a deliberate head-on collision. Equal-length trades are worth taking
    too, since a head-to-head collision kills both dragons regardless of length (see the death
    rules) -- and since enemy_lengths is a lower bound, "at least ours" can never be an
    overestimate: the real trade is always at least as good as what we can see."""
...
def execute_turn() -> None:
    ...
    # A pearl that will make us long enough to split (queens only care about this once
    # they're near their own scaling/capped growth threshold; see queen_split_threshold).
    if (target in pearls and body is not None and length >= 2 * SPLIT_SIZE - 1
            and ct.get_unit_count() < game.get_unit_limit()
            and (not is_queen or length + 1 >= queen_split_threshold(now))):
    ...
    # at most one safe way out. This is the survival override that lets a queen split even
    # below its growth threshold, purely to shed length and escape.
    ...
    if is_queen:
        ...
        # queens always split off a minimum-size (SPLIT_SIZE) child -- a bigger child isn't
        # worth it when the strategy leans on split frequency and kills over raw length.
        # The growth threshold scales with the round but is capped (see queen_split_threshold),
        # so a queen settles around a target length rather than growing all game. No geometry
        # gate any more (REQUIRE_OPPOSITE_HEADS=False) -- child_has_exit is what actually
        # matters, and it checks all 4 sides, not just one inherited direction.
        can_split = can_split_basic and length >= queen_split_threshold(now) and ct.can_split(SPLIT_SIZE)
        split_ok = can_split and child_has_exit(body, occupied)

        if not split_ok and in_trouble:
            # survival override: split to escape even if below the growth threshold
