...
SPLIT_SIZE = 2          # always split off the smallest legal piece
...
def split_ready(body, heading):
    """Would a size-2 split of `body` leave the child's head facing opposite to ours?

    The child is the last two segments reversed, so its head is our tail tip and it faces away
    from our body, i.e. along the step from the second-last segment to the last one."""
    if body is None or len(body) < 2 * SPLIT_SIZE:
        return False
...
def execute_turn() -> None:
    ...
    # A pearl that will make us long enough to split: if some shortest path reaches it moving in the
    # direction that leaves the heads opposite (growing keeps the tail where it is, so that is the
    # tail's own direction), take that path and be ready to split the turn after eating.
    ...
    # splitting outranks everything except not dying: split the moment it is legal and the heads
    # would part ways, and otherwise steer (below) to make that happen as soon as possible
    can_split = ct.can_split(SPLIT_SIZE) and body is not None
    if can_split and split_ready(body, heading) and child_has_exit(body, occupied):
        fleeing = safe and threatened(mp, enemies)
        if not fleeing:
            ct.do_split(SPLIT_SIZE)
            return
    ...
    # ... unless a split is legal and only needs lining up, in which case that comes first
    if can_split:
    ...
    def rank(i):
        ...
        aligned_next = False
        if want_split and body is not None:
    ...
    if DEBUG:
        ct.output_log("target", target, "move", DIRS[move].value)
